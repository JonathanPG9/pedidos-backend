...
const validationRegister = (data) => {
  const schema = joi.object({
    nombre: joi.string().min(3).max(15).required().regex((/^[A-Za-z]+$/)).messages({
      ...
      "object.regex": "El nombre solo admite caracteres alfabéticos",
      "string.pattern.base": "El nombre solo admite caracteres alfabéticos"
    }),
    apellido: joi.string().min(3).max(25).regex((/^[A-Za-z]+$/)).messages({
    ...
    password: joi.string().min(5).max(30).required().messages({
      ...
      "string.min": "La contraseña  Debe contener un minimo de 5 caracteres",
      "string.max": "La contraseña  Debe contener un maximo de 30 caracteres",
    }),
    ...
    dni: joi.string().min(8).max(8).regex(/^[0-9]+$/).messages({
    ...
    edad: joi.number().min(14).max(100).required().messages({
      ...
      "number.min": "Edad minima 14 años",
      "number.max": "Edad maxima 100 años",
    }),
    telefono: joi.string().min(10).max(10).regex(/^[0-9]+$/).messages({
      "string.empty": "Campo Telefono obligatorio",
      "string.min": "El Telefono Debe contener un minimo de 10 caracteres",
